...
class MissionController
{
  ...
  public function GenerateQuestion($qlevel)
  {
    ...
    switch ($this->model->type_id)
    {
    	...
    	case "7":  // addition 1-20
      		$firstDigit = rand(0, ($this->model->type_id == "4" ? 5 : 20));
      		$secondDigit = rand(0, ($this->model->type_id == "4" ? 5 : 20));
    	...
    	case "8":   // subtraction 1-20
      		$firstDigit = rand(0, ($this->model->type_id == "5" ? 10 : 20));
      		$secondDigit = rand(0, ($this->model->type_id == "5" ? 10 : 20));
    	...
    	case "9": // add or subtract with 3 whole numbers whose sum is less than or equal to 20
    		$firstNumber = rand(1, 18);
     		...
     		{
     			$thirdNumber = rand(1, ($firstNumber - $secondNumber));
     		}
     		else
     			$thirdNumber = rand(1, ($secondNumber - $firstNumber));
  ...
  public function UpdateGameLevel()
  {
    ...
    $sql = "select students.game_level from students where students.student_id=".($this->model->student_id);
  	$currentGameLevel = ($conn->query($sql)->fetch_array(MYSQLI_NUM)[0]);
  	
  	$sql1 = "select count(distinct student_mission_record.question_type_id) from question_type join student_mission_record on (question_type.question_type_id=student_mission_record.question_type_id) where question_type.grade_level = " .($currentGameLevel). " and student_mission_record.student_id = " .($this->model->student_id). " and student_mission_record.number_correct between 8 and 10";
	$missionsCompletedAtLevel = ($conn->query($sql1)->fetch_array(MYSQLI_NUM)[0]);
	
	$sql2 = "select count(distinct question_type.question_type_id) from question_type where question_type.grade_level=".($this->model->student_id);
	$totalMissionsAtLevel = ($conn->query($sql2)->fetch_array(MYSQLI_NUM)[0]);
	
	if ($missionsCompletedAtLevel == $totalMissionsAtLevel)
	{
		
		$currentGameLevel = $currentGameLevel + 1;
		$_SESSION["current_student_game_level"] = $currentGameLevel;
		($conn->query("update students set students.game_level=".($currentGameLevel)." where students.student_id=".($this->model->student_id)));
	}
	$conn->close();
  }
}
